									...
									identification_t *id, message_t *message)
{
	...
	chunk_t sign1, sign2; /* Socrates addded */
								...
								&octets, schemes))
	{
		...
		while (enumerator->enumerate(enumerator, &params))
		{
			if (!private->sign(private, params->scheme, params->params, octets,
							   &sign1) ||
				!build_signature_auth_data(&sign1, params))
			{
			...
			if (!private->sign(private, params->scheme, params->params, octets,
							   &sign2) ||
				!build_signature_auth_data(&sign2, params))
			{
				DBG2(DBG_IKE, "unable to create %N signature for %N key",
					 signature_scheme_names, params->scheme, key_type_names,
					 private->get_type(private));
			...
			chunk_t double_signature = chunk_cat("cc", sign1, sign2); 

			DBG2(DBG_IKE, "Double Signature Content: %.*s", double_signature.len, double_signature.ptr);

			// Display sign1
			DBG2(DBG_IKE, "Sign1 Content: %.*s",
				sign1.len, sign1.ptr);

			// Display sign2  
			DBG2(DBG_IKE, "Sign2 Content: %.*s",
				sign2.len, sign2.ptr);

			add_auth_to_message(message, AUTH_DS, double_signature, FALSE);
	...
	if (params)
	{
		if (params->scheme == SIGN_RSA_EMSA_PSS)
		{
			...
			DBG1(DBG_IKE, "Socreates authentication of '%Y' (myself) with %N_%N_SALT_%zd "
		...
		{
			DBG1(DBG_IKE, "Socreates double authentication of '%Y' (myself) with %N %s", id,
							 ...
							 identification_t *id, message_t *message)
{
	...
	DBG1(DBG_IKE, "Socrates classic authentication of '%Y' (myself) with %N %s", id,
	...
	private_pubkey_authenticator_t *this, message_t *message)
{
	...
	chunk_t received_sign1, received_sign2;

	for (size_t i = 2; i < auth_data.len; i++) {
	if (auth_data.ptr[i] == 'c') {
		received_sign1.ptr = auth_data.ptr + 2;
		received_sign1.len = i - 2;

		received_sign2.ptr = auth_data.ptr + i + 1; 
		received_sign2.len = auth_data.len - i - 1;
		break;
	}
	}

	// Display received_sign1
	DBG2(DBG_IKE, "Received Sign1 Content: %.*s",
		received_sign1.len, received_sign1.ptr);
	
	// Display received_sign2
	DBG2(DBG_IKE, "Received Sign2 Content: %.*s",
		received_sign2.len, received_sign2.ptr);
	...
	while (enumerator->enumerate(enumerator, &public, &current_auth))
	{
		if (public->verify(public, params->scheme, params->params, octets,
						   received_sign1) && public->verify(public, params->scheme, params->params, octets,
						   received_sign2) &&
			is_compliant_cert(current_auth))
		{
			if (auth_method != AUTH_DS)
			{
				DBG1(DBG_IKE, "Socrates 1 double authentication of '%Y' with %N successful", id,
					 auth_method_names, auth_method);
			}
...
#if DEBUG_LEVEL >= 1
				rsa_pss_params_t *pss = params->params;
				DBG1(DBG_IKE, "Socrates 2 authentication of '%Y' with %N_%N_SALT_%zd "
			...
			{
				DBG1(DBG_IKE, "Socrates 3 authentication of '%Y' with %N successful", id,
					 signature_scheme_names, params->scheme);
			}
			status = SUCCESS;
